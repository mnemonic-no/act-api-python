...
def parseargs():
    ...
    parser.add_argument(
        "--userid", type=int, dest="user_id", required=True, help="User ID"
    )
    parser.add_argument(
        "--act-baseurl", dest="act_baseurl", required=True, help="API URI"
    )
...
def request(method, user_id, url, **kwargs):
    return requests.request(
        method, url, headers={"ACT-User-ID": str(user_id)}, **kwargs
    )
    ...
    **kwargs
    ...
    if not filename:
        filename = "{}_{}_{}.json".format(
            method.lower(), endpoint.replace("/", "_"), status_code
        )
    ...
    if expected_status_code and not (expected_status_code == status_code):
        sys.stderr.write(
            "Skipping, {} did not match expected status code ({} != {})\n".format(
...
def create_object_type_mock(name, filename):
    create_mock(
        ...
        "POST",
        "v1/objectType",
        json={"name": name, "validator": "RegexValidator", "validatorParameter": r".+"},
        filename=filename,
...
# Get Object Types
object_types = create_mock(
    args.act_baseurl, args.user_id, "GET", "v1/objectType", DATADIR
)
...
# Get Threat Actor object type
threat_actor_alias = [
    ft for ft in fact_types["data"] if ft["name"] == "threatActorAlias"
][0]
...
facts = create_mock(
    ...
    "POST",
    "v1/fact/search",
    json={"factType": ["seenIn"], "factValue": ["report"], "limit": 1},
...
create_mock(
    ...
    "POST",
    "v1/origin",
    json={"name": "my-origin", "description": "My origin", "trust": 0.8},
